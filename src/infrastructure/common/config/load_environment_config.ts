...
function envDetector(): string | string[] {
  switch (process.env.NODE_ENV) {
    ...
    case 'production':
      return ['config/production.local.js', 'config/production.js'];
...
export default () => {
  if (configFilename instanceof Array)
    configFilename = existsSync(join('./', configFilename[0])) ? configFilename[0] : configFilename[1];
